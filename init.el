...
;; --- Clipboard bridge --------------------------------------------------------
;; Use xclip for the clipboard, whether in a graphical frame or a text terminal
;; under `ssh -Y' (xclip talks to the forwarded X, encrypted, no `-listen tcp'
;; or `xhost'). gnome-terminal/VTE (0.80 here) does not honor OSC 52 clipboard
;; writes, so xclip over `ssh -Y' is the working path; connect that way and make
;; sure DISPLAY is set (e.g. localhost:10.0) so clipboard-out works.
(defun hcv-setup-clipboard (&optional frame)
  "Enable xclip for clipboard integration."
